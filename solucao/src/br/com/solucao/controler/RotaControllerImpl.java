		...
		if(mapa == null) {
			...
			for (Rota rota : malha.getRotas()) {
				...
				if(mapa.getLocalizacoes().get(rota.getOrigem()) == null) {
					...
					erro = new Erro(new Rotas(mapaNovo,rota.getOrigem(),rota.getDestino()), Constantes.LOCALIZAÇÃO_ORIGEM_NAO_ENCONTRADO);
					erros.add(erro);
					rotaValida = Boolean.FALSE;
				}
				if(mapa.getLocalizacoes().get(rota.getDestino()) == null) {
					...
					mapaNovo.setId(mapa.getId());
					mapaNovo.setNome(mapa.getNome());
					erro = new Erro(new Rotas(mapaNovo,rota.getOrigem(),rota.getDestino()), Constantes.LOCALIZAÇÃO_DESTINO_NAO_ENCONTRADO);
					erros.add(erro);
					rotaValida = Boolean.FALSE;
				}
